...
class Ensembl:
    ...
    def query_omim(self, omim_morbid):
        ...
        keys = ['OMIM_morbid']
        key_values = {'OMIM_morbid': omim_morbid}
    ...
    def query_ensemble(self, ensembl_id):
        """Queries EnsEMBL based on the Ensembl_gene_id. Data from EnsEMBLdb will overwrite
        the client data.
        An identifiers should yield one result from EnsEMBLdb.
    ...
    def query_transcripts_omim(self, omim_id=None, gene_id=None):
        ...
        JOIN object_xref ox on ox.ensembl_id = g.gene_id AND ensembl_object_type = 'Gene'
        JOIN xref xx on xx.xref_id = ox.xref_id AND xx.external_db_id IN (1500, 1510, 1520)
